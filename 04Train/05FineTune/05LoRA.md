...
$$
W_{text}^{^{\prime}}=W_{text}+ \alpha_{text} W_{text}
$$

这种双组件微调机制建立了一种分工明确的协同关系。文本编码器 LoRA 负责语义层面的定义，即将一个下游任务概念固化为模型语言空间中的一个稳定嵌入；而 U-Net LoRA 则负责视觉层面的执行，即学习如何将这个新的语义嵌入，在图像生成过程中将其渲染为一致的视觉特征。该方法能够在仅改动极少量参数的前提下，实现对特定主体或风格的高保真定制，同时最大程度地保留了预训练模型的泛化能力。
...
从最初为解决全参数微调高昂成本而提出的改进构思，到如今枝繁叶茂的算法家族，LoRA 的发展历程本身就是对**效率**与**效果**这对核心矛盾不断探索与权衡的缩影。 它深刻地改变了我们与大模型互动的方式，极大地降低了定制化、适配特定任务的门槛。

最初的 LoRA 凭借其“低秩近似”的核心思想，以极小的参数量实现了接近全量微调的效果，并因其“即插即用”和“无损推理”的特性迅速成为主流。 但这一开创性的工作也并非完美，后续的变体正是针对其暴露出的不同层面的局限性，进行了精准而深刻的优化。QLoRA 通过量化技术，将微调的硬件门槛降至消费级，进一步推动了技术的普及；AdaLoRA 则揭示了不同模块重要性的差异，通过动态分配参数预算，实现了更精细化的资源调控；DoRA 更进一步，从权重分解的全新视角剖析了 LoRA 与全量微调在学习机制上的根本差异，通过解耦量级和方向的更新，显著缩小了两者间的性能鸿沟。

与此同时，LoRA+ 聚焦于训练动态本身，通过修正梯度尺度不对称问题，以零额外成本加速了收敛并提升了性能。而 MoE-LoRA 则将视野投向了“后微调时代”的应用，探索如何高效、无损地组合多个专家 LoRA，为实现复杂、复合能力的动态生成开辟了新的道路。这一系列演进不仅是技术上的迭代，更是我们对大模型微调本质理解的不断深化：从最初简单地“近似更新”，到后来“智能化地分配更新预算”，再到“解构更新的内在机制”，直至“优化更新过程”与“组合更新结果”。

展望未来，LoRA 的发展脉络预示了参数高效微调的几个可能方向。首先，随着模型规模的持续扩大，对效率的追求将永无止境，结合剪枝、稀疏化等更多技术，探索更为极致的参数压缩方法仍将是研究热点。 其次，自动化与智能化将是关键，如何让模型自适应地学习最佳的微调策略，例如自动确定秩、选择微调模块、甚至动态组合不同的 PEFT 方法，将是提升易用性和效果的重要方向。最后，如何更好地处理多任务、多模态的融合微调，以及如何在保证高效的同时，更好地解决**灾难性遗忘**等根本性难题，也将是 LoRA 及其衍生技术需要持续面对和攻克的挑战。
...
- ChatLaw：https://github.com/PKU-YuanGroup/ChatLaw/tree/main
- Bencao：https://github.com/SCIR-HI/Huatuo-Llama-Med-Chinese
- 定向物体图像识别：https://arxiv.org/abs/2406.02385
- Latent Diffusion Model：https://arxiv.org/abs/2112.10752
- LoRA on Stable Diffusion：https://github.com/cloneofsimo/lora
