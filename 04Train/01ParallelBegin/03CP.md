...
CP 正是要解决 SP 中的 Self-Attention 序列并行问题，CP 通过 Flash Attention2 分块运算和分块结果修正的方式，成功地打破了这一限制。此外，CP 通过环形网络在设备间传递 KV 值来获取分块运算结果，其原理类似 Ring Attention。
...
现在我们已经了解了CP的运行流程，但是CP的优化不止于此，CP还通过数据对称重组的方式丢弃了无效的掩码运算，对因果掩码（Casual Mask）进行了优化。

上面我们介绍到，Q/K/V 在计算Attention时会有 sq/cp_size 的操作，因果掩码计算优化与之相似，但是是对Q/K/V进行 2*cp_size 的等分操作然后再进行组合。我们仍以上文 cp_size = 4为例，如图所示。由于在Transformer的Attention里，只有score矩阵会用到因果掩码计算，所以这里我们只关注score矩阵的计算。

![](./images/03.ContextParallelism10.png)

<center>Q/K矩阵的数据拆分和对称重组</center>

首先进行数据拆分，将Q/K按序列维度等分为 2*cp_size = 8 份，编号从0到7。而后是关键的一步，对称组合。要把8份Q/K，两两对称组合成4个新的子块，这个组合是对称的，从两端向中间配对，得到 Q0，Q1，Q2，Q3 和 K0，K1，K2，K3。

最后，由于要与计算数据进行映射，原始掩码矩阵要切分成 8×8 个小块，后续当不同的子块Qx和Kx组合进行计算后进行掩码操作时，需要提取的掩码矩阵子块的索引由Q的索引和K的索引两两组合得到。如图所示（待画），Q1包含原始索引 [1,6]，K2包含原始索引 [2,5]，所以需要提取的Mask子块是：12, 62, 15, 65。

因为对称重组后，每个 rank 手里的 Q 和 K 都是两端往中间抽的2个 token，所以 8×8 的 causal-mask 被切成 8×8 个小方块后，每个 rank 在 FA2 内层循环里只会遇到如图所示三种子矩阵：
...
而要剔除 causal-mask 中的无用计算，具体步骤如下：
1. 首轮循环（i = 0），启用标准 causal 模式的 FA2，完整保留 Q、K、V 全段。
2. 后续循环且 i ≤ rank_idx，判定为“左虚右实”——K、V 截掉 sequence 后半段，仅以无 mask 方式参加运算。
3. 循环继续且 i > rank_idx，转为“上空下实”——Q 截掉 sequence 前半段，K、V 全段出场，仍以无 mask 方式完成计算。

依旧使用 cp_size=4 为例，当设备rank1进行FA2求解时：
1. 当 i = 0时，属于首轮特殊处理：先以 causal 模式载入完整块，随后识别到左下半区空白，于是 K/V 只保留前半段（seq//2），Q 维持全长，切换为无掩码运算。
2. 当 i == rank_idx（此时 i=1）时，恰好位于对角线，判定为必须保留完整因果约束。Q、K、V 均不裁剪，使用标准 causal_mask 计算。
3. 当 i > rank_idx（i=2,3）时，进入右上半区，空白位于上半阵。此时 Q 截去前半段（seq//2），K/V 保持全长，运算再次退化为无掩码路径，上空区域被直接跳过。

通过上述四次判定，循环内实际送入芯片的矩阵尺寸始终为“实部”大小，掩码乘法被彻底省略，而因果语义仍保持完整。
