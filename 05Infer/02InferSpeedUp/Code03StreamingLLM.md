...
```
    ...
    Softmax 后的权重: tensor([0.8007, 0.0399, 0.0399, 0.0399, 0.0399, 0.0399])
    初始 token 获得的注意力比例: 80.07%
...
```
    ...
    处理 Token 1: 缓存大小=2, 注意力权重最大值=0.667
    处理 Token 2: 缓存大小=3, 注意力权重最大值=0.400
    处理 Token 3: 缓存大小=4, 注意力权重最大值=0.343
    处理 Token 4: 缓存大小=5, 注意力权重最大值=0.346
    处理 Token 5: 缓存大小=6, 注意力权重最大值=0.345
    处理 Token 6: 缓存大小=6, 注意力权重最大值=0.264
    处理 Token 7: 缓存大小=6, 注意力权重最大值=0.264
    处理 Token 8: 缓存大小=6, 注意力权重最大值=0.334
    处理 Token 9: 缓存大小=6, 注意力权重最大值=0.287
    
     最终缓存包含的 token 原始位置: [0, 1, 6, 6, 6, 6]
...
```
    ...
    已处理 Token 总数: 15
    当前缓存大小: 12（目标容量：12）
    缓存中 Token 的原始位置: [0, 1, 2, 3, 7, 8, 9, 10, 11, 12, 12, 12]
    重新编码后的相对位置: [0, 1, 2, 3, 4, 5, 6, 7, 8, 11]
    ------------------------------
    已处理 Token 总数: 20
    当前缓存大小: 12（目标容量：12）
    缓存中 Token 的原始位置: [0, 1, 2, 3, 12, 12, 12, 12, 12, 12, 12, 12]
    重新编码后的相对位置: [0, 1, 2, 3, 11]
    ------------------------------
    演示完成！StreamingLLM 成功处理了 20 个 Token（远超初始缓存容量）
    最终缓存大小: 12（稳定在目标容量）
    最终缓存中的 Token 原始位置: [0, 1, 2, 3, 12, 12, 12, 12, 12, 12, 12, 12]
