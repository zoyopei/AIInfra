...
通信软件在高性能计算领域也是发挥至关重要的作用，从卡间互联到节点互联各个层级的通信技术也历经发展。

### 发展历程：MPI统治时代（1990s-2010s）

#### 消息传递接口 MPI：HPC 通信标准（Open MPI/Intel MPI），支持P2P与集合通信。

MPI（Message Passing Interface）是一种跨语言的通讯协议和凡是标准，定义了多个原语的消息传递接口，主要被用于多进程间的通信，支持丰富的消息通信机制，基于TCP/IP实现点对点、组播和多播模式。MPI系统通信方式建立在点对点的通信上，同时在端到端通信的基础上在一组进程内用通讯原语支持集合通信。

P2P指点对点通信，是两个进程间通信，用于控制同步或数据传输。点对点通信分为同步（阻塞）和异步（非阻塞）。在同步阻塞通信方式中，MPI_Send 返回意味进程发送数据结束，进程缓冲可以重用/覆盖，但不代表 Receiver 收到数据，MPI_Recv 返回意味着数据已经接收到进程的缓冲区，可以使用。在异步非阻塞通信方式中，进程完成send/receive操作马上返回，继续后续阶段的计算。当程序要求操作必须确认完成时，调用相应测试接口（e.g. MPI_Wait）阻塞等待操作完成。异步编程相对复杂，但使得计算和通信可以一定程度并行，降低数据同步带来的运行时开销。

如图为P2P过程演示。

![P2P](images/03TrendSoft22.png)

集合通信里，MPI原语在进程启动和收发数据里发挥重要作用。Broadcast是用于分发数据的通信原语，当 MPI 进程启动后，每个 Process 会分配唯一序号 Rank。集合通信需要指定一个协调者（比如Rank 0 Process，一般称为 ROOT），由其负责将数据发送给所有进程。如下图以MPI_BCAST 为例，它将数据从根进程发送到所有其它进程。所有进程都调用 MPI_BCAST。虽然每个进程都调用 MPI_Bcast，但根进程负责广播数据，其它进程接收数据。

如图为Broadcast分发数据过程。

![Broadcast分发数据过程](images/03TrendSoft23.png)

Gather是用于采集数据的通信原语，MPI_Gather让每个进程将数据发给根进程。Barrier是用于同步数据进程的原语。某些场景下，多个进程需要协调同步进入某个过程。MPI提供了同步原语例如MPI_Barrier。所有进程调用MPI_Barrier，阻塞程序直到所有进程都开始执行这个接口，然后返回。Barrier作用就是让所有进程确保 MPI_Barrier 之前的工作都已完成，同步进入下一个阶段。


如图为Gather收集数据过程。

![Gather收集数据过程](images/03TrendSoft24.png)

Intel MPI和Open MPI都是流行的MPI实现，它们支持多种操作系统平台，但在某些平台上的支持程度和性能优化可能有所不同。Intel MPI是由英特尔公司开发的MPI库，它在英特尔处理器和英特尔架构的集群上表现出色，提供了许多高级优化和调试功能。Intel MPI支持Windows、Linux和Mac OS等操作系统，并且在这些平台上都有良好的性能表现。特别是在使用Intel处理器的系统上，Intel MPI能够提供额外的性能优化。Open MPI是一个开源的、高性能的MPI库，它支持多种操作系统和硬件架构。Open MPI可以在Windows、Linux和Mac OS上运行，并且支持MPI-3标准的最新功能。Open MPI在具有共享内存的Linux系统上表现良好，并且可以支持单副本优化。对于具有Mellanox InfiniBand的Linux系统，Open MPI提供了良好的性能，尤其是在支持MPI-3或MPI_THREAD_MULTIPLE的发行版上。总体来看，Intel MPI在使用Intel处理器的系统上提供了优化，而Open MPI则在开源实现和跨平台支持方面表现更佳。





#### 性能优化 ：RDMA 技术将延迟降至 1μs 级（比如 InfiniBand Verbs API）。

远程直接数据存取(remote direct memory access, RDMA)技术允许一台计算机的应用程序直接访问另一台计算机的内存，而无需中介，绕过远程主机操作系统和内核访问其内存中数据的技术，概念源自于 DMA 技术。在DMA技术中，外部设备（PCIe 设备）能够绕过CPU直接访问本地主机的内存，不仅可以访问本地主机的内存，还能够访问另一台主机上的用户态内存（通俗的看成是远程的DMA技术）。


下图为RDMA概念图。RDMA使得计算机无需中介直接访问另一台计算机内存。

![RDMA概念](images/03TrendSoft25.jpg)


这种机制的优点包括：

1. 低延迟：通过绕过操作系统和 CPU，减少了数据传输的延迟。

2. 高吞吐量：可以实现更高的数据传输速率，适合大规模并行计算。

3. 降低 CPU 负载：CPU 不再需要处理每一个数据包的传输，释放计算资源用于其他任务。

目前RDMA有三种不同的硬件实现，它们都可以使用同一套API来使用，但它们有着不同的物理层和链路层：

1. InfiniBand(IB): 基于 InfiniBand 架构的 RDMA 技术，需要专用的 IB 网卡和 IB 交换机。从性能上，很明显Infiniband网络最好，但网卡和交换机是价格也很高。基于IB实现的RDMA将数据传输延迟至 1μs 级别。
...
3. iWARP：Internet Wide Area RDMA Protocal，基于 TCP/IP 协议的 RDMA 技术(在现有TCP/IP协议栈基础上实现RDMA技术,在TCP协议上增加一层DDP)，由 IETF 标 准定义。iWARP 支持在标准以太网基础设施上使用 RDMA 技术，而不需要交换机支持无损以太网传输，但服务器需要使用支持iWARP 的网卡。与此同时，受 TCP 影响，性能稍差。
...
#### UCX（Unified Communication X）：支持 MPI/OpenSHMEM/PGAS 等多编程模型。


Unified Communication X (UCX) 是一个高性能、开源的通信库，旨在为各种并行计算应用提供统一的通信接口。UCX 支持多种编程模型，包括但不限于 MPI、OpenSHMEM 和 PGAS（Partitioned Global Address Space）模型。UCX主要提供RDMA通信的高级API以加速应用开发，在隐藏底层细节的同时还实现了一些优化和最佳实践以保持高性能和可扩展。除了RDMA（IB和RoCE），UCX也支持包括TCP，GPU和共享内存等在内的其他传输层。UCX是一个开源项目，通过https://github.com/openucx/ucx访问。

UCX 整体分为两层：上层的 UCP 接口和底层的 UCT 接口。
...
1. 自动选择最优的传输层和设备。一个主机节点上往往有多张网卡和每张网卡可支持多种传输协议。UCX可以根据策略选择最优的设备和传输协议，而在最优的选择不可用时，切换到其他的组合。

2. 不同大小的消息采用不同的协议。不同长度的消息采用不同的协议以达到最大的性能。比如RDMA传输过程中，小消息采用eager协议能达到较好的延迟，大消息则采用Rendezvous协议能达到较大的吞吐率。

3. 内存注册缓存:在RDMA传输中，为了应用和设备能够直接通信，应用需要注册内存并告知硬件，然而内存注册/注销操作的开销比较大。内存注册缓存则在内存第一次使用时注册，而不用时并不注销而是存入缓存，再次使用时，直接从缓存获取而不需要再次注册，只有在缓存不够或者内存释放时才会真正注销。

4. 端口聚合：Multi-rail & RoCE LAG。多个设备可以聚合成一个设备作为负载均衡或者冗余备份时使用。

这里简单介绍编程模型OpenSHMEM和PGAS。OpenSHMEM是单程序、多数据（single program multiple data，SPMD）编程模型，特点是强调单边通信以及数据移动与同步解藕，是提供高性能、高可扩展性通信和同步程序的库。OpenSHMEM程序由松散同步的进程组成，这些进程也叫作处理单元（Processing Element，PE）。所有PE同时开始并执行相同的程序，通常在各自集合问题的子域上执行操作，并周期性地与其他PE通信。

PGAS是一种数据并行模型，被称为“全局地址空间分区”（Partitioned Global Address Space (PGAS)）模型。在这种编程模型里，地址空间被认为是全局的。大多数的并行工作聚焦于在数据集上的操作。数据集通常被组织成为常用的结构，例如数组，数立方等。一系列任务在同一块数据结构上操作，但是每个任务却操作在该数据结构的不同分区上。在共享内存的架构下，所有的任务通过全局内存方式来对数据进行存取；在分布式内存架构下，根据任务分配，全局数据结构在物理或者逻辑上被进行分割。目前，基于数据并行/PGAS模型，有如下几个相对有名的实现：
...
#### 云原生通信：gRPC 与 RDMA 结合降低跨容器通信开销。


RDMA（Remote Direct Memory Access），可以简单理解为网卡完全绕过CPU实现两个服务器之间的内存数据交换。其作为一种硬件实现的网络传输技术，相对TCP有着显著的优势，可以大幅提升网络传输效率，帮助网络IO密集的业务（比如分布式存储、分布式数据库等）获得更低的时延以及更高的吞吐。

在RDMA传输的适配中，RPC业务场景调用首当其冲。RPC 的全称是 Remote Procedure Call，中文为远程过程调用。它是一种技术理念，允许你像调用本地函数一样去调用一个位于远程服务器上的函数或方法，而无需显式地处理网络通信的细节（如套接字、序列化、反序列化、网络传输等）。核心思想是“屏蔽复杂性”，开发者只需要关注业务逻辑（调用哪个函数、传递什么参数），而不需要关心数据是如何通过网络打包、发送和接收的。所有这些底层工作都由 RPC 框架自动完成。

在RPC over RDMA场景下，可以直接在服务器内存和客户端内存之间传输RPC数据。gRPC是一个现代的高性能、开源、通用的 RPC 框架，为了让gRPC使用RDMA技术，可以通过修改内核和网络栈，使得 gRPC 能够透明地运行在 RoCE 网络上。或者业务侧将原有使用的RPC（大部分是GRPC）调用改为ORPC调用，在这种场景下业务和传输更像是两个独立的模块，通过SDK的方式进行调用，所以适配起来改造的代码并不多，通常是业务层面修改调用RPC的接口方式。
...
提高计算、存储和网络资源之间的电气连接性能变得越来越困难，所以服务器互联通信迈向光 I/O领域进行探索，以处理数据中心飙升的带宽需求。

硅光子技术是一种通过硅基工艺将光子器件与电子元件集成的新型光通信技术，利用光信号替代传统铜导线传输数据。其核心技术包括混合硅激光器、光波导通路和光电共封装（CPO），可实现传输速率提升100倍以上、功耗降低50%的突破。

英特尔基于英特尔内部硅光子学技术开发了一款 4 Tbps 双向全集成 OCI 小芯片，以满足 AI 基础设施对带宽的巨大需求并实现未来的可扩展性。此 OCI 小芯片或模块包含一个集成激光器的硅光子集成电路 （PIC）、一个电气 IC （EIC） 以及一条用于集成可拆卸/可重复使用光连接器的路径。OCI 小芯片可以与下一代 CPU、GPU、IPU 和其他具有高带宽需求的片上系统 （SOC） 共同封装。第一个实施为提供多太比特光连接铺平了道路，海岸线密度比 PCIe Gen6 提高了 >4 倍，能效为 <3pJ/bit，延迟为 <10ns （+TOF），并且覆盖范围大于 100 米。



#### 自适应路由：基于拓扑感知的动态路径选择（e.g. SLURM Gang Scheduling）。

将基于拓扑状态感知的动态路径等高级算法运用于作业调度也是高性能计算领域通信技术发展趋势之一。Simple Linux Utility for Resource Management （SLURM） 是一个开源集群管理和作业调度系统。目前，世界上许多超级计算机和计算集群使用SLURM。SLURM 是并行计算机中用于执行并行作业的重要组成部分。最初，SLURM 是作为资源管理器开发的。随着时间的推移，它通过其调度插件演变成一个功能强大的作业调度程序。当提交到集群的作业数量超过可用资源时，作业调度功能显得很重要。目前，SLURM 支持复杂的作业调度算法，并针对网络拓扑、公平共享、高级预留、抢占、帮派调度等进行优化。

Slurm 支持时间片gang Scheduling，可将多个作业分配至同一分区的相同资源上，并通过交替挂起的方式，使这些作业在设定的时间片内轮流独占资源。该机制还与抢占式调度相结合，当高优先级分区或具有抢占QOS的作业需要资源时，可通过挂起（SUSPEND）低优先级作业来实现抢占，而帮派调度器负责在适当时机恢复被挂起的作业。这种调度策略显著提升了系统响应速度和资源利用率。
